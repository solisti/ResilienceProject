function Step3_testing_set(matrixname, bitflip_iter)
...
M = 50; % number of experiments, each injects error at a random vector element
...
new_error = ['./matrices/', matrixname, '_testerror.mat'];
...
% for i = injections
    bitflip_iter = 1;
    result_filename = ['./data/', matrixname, '/Step3_', matrixname, '_iter=', num2str(bitflip_iter),'_test', '.dat'];
    rel_grad_filename = ['./matrices/', matrixname, '_iter=', num2str(bitflip_iter), '_gradient_relative_test.mat'];
    abs_grad_filename = ['./matrices/', matrixname, '_iter=', num2str(bitflip_iter), '_gradient_absolute_test.mat'];
    xval_filename = ['./matrices/', matrixname, '_iter=', num2str(bitflip_iter), '_xval_test.mat'];
    for m = 1:M
        ...
        result = [N,flag,iter,bitflip_pos,diff_v,A_row_2norm(bitflip_pos),noerror_converge,converge, max(p),grad_abs(bitflip_pos), grad_rel(bitflip_pos), standard_gradient(bitflip_pos), xval(bitflip_pos), bitflip_iter];
