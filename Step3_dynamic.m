function Step3_dynamic(matrixname, bitflip_iter)
...
drawnow('update');
[N, ~] = size(A);
result_filename = ['./data/Step3_', matrixname, '_iter=', num2str(bitflip_iter), '_dynamic','.dat'];
...
new_error = ['./matrices/', matrixname, '_newerror_dynamic.mat'];
% load iterations file
iter_filename = ['./matrices/', matrixname, '_convergence_dynamic.mat'];
...
%         num_elements = ceil(noerror_converge * fraction);
        ...
        indices = datasample([1:noerror_converge], M); % find the interations we want to inject error in
